...
// IconKind is used to specify the usage of an icon (mirrors webview/webview_go#47).
type IconKind int
...
type WebView interface {
	...
	// thread. On GTK/Linux the kind parameter is ignored (the WM scales a
	// single icon itself). Currently a no-op on macOS.
	// Mirrors the API from webview/webview_go#47, extended to Linux.
